                            ...
                            <table class="table table-striped" id="table-2">
                                ...
                                <tbody>
                                    @forelse ($dokumen_sidang as $dokumen)
                                        ...
                                        <td>
                                            @if($dokumen->pem2 && $dokumen->pem2->dosen_id == $dosen->id &&
                                            $dokumen->pem2->status_dokumen)
                                        ...
                                        <td>
                                            {{ $dokumen->pem2->komentar_dokumen }}</>
                                        </td>

                                        <td>
                                            @if($dokumen->pem2 && $dokumen->pem2->dosen_id == $dosen->id && $dokumen->pem2->status_dokumen)
                                            <div class="mb-2 mt-2">
                                            <span class="badge badge-success">Disetujui</span>
                                            </div>
                                            @else
                                            <div class="mb-2">
                                                <form method="POST" action="{{ route('dokumenSidang.status2', $dokumen->id) }}" class="mb-2 mt-2">
                                                    @csrf
